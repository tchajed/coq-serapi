...
let ser_loop ser_opts =
  ...
  let rec loop ctx =
    try
      ...
      let ctx = process_serloop_cmd ~ctx ~pp_ack ~pp_answer ~pp_err ~pp_feed scmd in
      if not (is_cmd_quit scmd) then loop (incr_cmdid ctx)
    with Sys.Break -> loop (incr_cmdid ctx)
  in loop ctx
